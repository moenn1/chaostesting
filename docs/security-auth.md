...
- the success payload returns terminal `STOPPED` state, `endedAt`, and assignment counters
...
At dispatch time the control plane snapshots healthy matching agents into persisted run assignments, and stop actions update those assignments to `STOPPED` alongside the parent run.
